...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const patientId = calendarContainer.getAttribute("data-patient-id");

    if (patientId) {
        renderCalendar(patientId);
    } else {
        console.error("Errore: patientId non trovato!");
    }
});

async function fetchAppointments(patientId) {
    try {
        let response = await fetch(`/psychotherapist-appointments/${patientId}`);
        if (!response.ok) {
            throw new Error(`Errore nel recupero degli appuntamenti. Status: ${response.status}`);
        }
        let appointments = await response.json();
        console.log('Appuntamenti recuperati:', appointments);
...
async function renderCalendar(patientId) {
    ...
    let appointments = await fetchAppointments(patientId);
...
function prevMonth() {
    ...
    const patientId = document.getElementById("calendar").getAttribute("data-patient-id");
    renderCalendar(patientId);
}
